...
function generateSEBConfig (courseID) {
    ...
    const regexArr = [
      '([\\w\\d]+\\.)?canvas\\.kth\\.se(\\/)?$',
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/assignments(\\/.*)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/external_tools\\/retrieve\\?display=full_width&amp,url=https%3A%2F%2Fkth.quiz-lti-dub-prod.instructure.com(.*)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/modules(\\/.*)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/modules/items/([0-9]+)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/modules\\#module_([0-9]+)$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/pages\\/([a-zA-Z0-9_.-]+)\\?module_item_id=([0-9]+)$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/quizzes(\\/.*)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/student_view(\\/.*)?$`,
      `([\\w\\d]+\\.)?canvas\\.kth\\.se\\/courses/${courseID}\\/test_student(\\/.*)?$`,
